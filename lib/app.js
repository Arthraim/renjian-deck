...
var ada = function () {
    ...
    var REFRESH_MS = 120000,
        ...
        //SRCH_URL = "http://search.renjian.com/search?q=%23",
        ...
        gLoaders = {}, // home, repl, pblc, msgs, updt, dmsg, cfav, dfav, user
        gStatuses = {},
...
//		gDmUser = id_user[i];
    ...
    function onUpdate() {
        ...
        vars = new air.URLVariables();
        vars["source"] = "renjian-deck"; //gAppName;
	...
	function generateHtml(tweet) {
		var dvtweet, id, user, name, gender, img, when, favorited, txt, i, tailMatch,
			...
			status_type, link_url, original_url, link_title, link_desc;
        ...
        txt = rawtxt
			...
			.replace(/@([\w\u4e00-\u9fa5]+)/g,
                "@<a href='" + ROOT_URL + "$1' class='link'>$1</a>");
            /* hashtag is not nesessary
			.replace(/\#(\w+)/g,
    ...
    */
	...
	function SocketOnError(e) {
		myTrace("called SocketOnError!");
		gMsg.say("推送服务发生错误！");
		myTrace(e);	
	}
	
	function SocketOnConnect() {
		myTrace("called SocketOnConnect!");
	}
	function SocketOnClose() {
		myTrace("called SocketOnClose!");
		gMsg.say("推送服务已断开！");
	}
	
	function SocketOnData(json) {
		myTrace("called SocketOnData!");
		myTrace(json);
		var count = 0, updatedCvsts = new Array();
	...
	function onConversation(event){
		...
		myTrace("called onConversation: " + id_cvst);
    ...
    function ini() {
        ...
        appUpdater.updateURL = "http://arthraim.cn/update.xml"; 
        appUpdater.delay = 1;
