...
var ada = function () {
    ...
    function showLogin(err) {    
        ...
        $("#inurl,#infilter,#urltitle,#imgurl").val("").blur();
        $("#spq").text(function () {
            var q = [
                "饭否？人间有饭吃！",
                "来人间嘀咕几句吧～",
...
//                $("#dvupdatecount").text(len + dmsgMatch[0].length);
    ...
    function showUpdateArea(show) {
        ...
        if (show) {
            ...
            $("#inurl,#dvurl,#urltitle,#dvurlcncl,#imgurl").fadeOut(speed, function () {
...
//        gDmUser = id_user[i];
    ...
    */
    ...
    function init() {
		...
		$("#imgurl").keypress(function (e) {
            if (e.charCode === 13) sharePicture();
        });
        $("#infilter").keyup(doFilter);
        $("#inurl,#infilter,#urltitle,#imgurl")
            .focus(function () {
		...
		$("#bnsharepic").click(function () {
			$("#inurl,#dvurl,#urltitle,#dvurlcncl").hide();
			if ($("#imgurl").is(":hidden")) {
				gStatusType = "PICTURE";
				$("#imgurl").fadeIn(speed = 200);
				$("#imgurl").focus();
			} else if ($("#imgurl").is(":visible")) {
				gStatusType = "TEXT";
                $("#imgurl").fadeOut(speed = 200);
			}
		});
